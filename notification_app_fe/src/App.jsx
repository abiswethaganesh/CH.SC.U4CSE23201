...
const NotificationDashboard = () => {
  ...
  useEffect(() => {
    Log("frontend", "info", "component", "Dashboard loaded");
    let mounted = true;
    setIsLoading(true);

    (async () => {
      try {
   const res = await fetch('/api-proxy/evaluation-service/notifications', {
  headers: {
    Authorization: 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..-DB5qsPslkERYpcnN12stLVx8mMzqPxL_OFWb5IaYzc'
  }
});
  ...
  createdAt: n.Timestamp
        ...
        console.error(err);
        console.warn("External API blocked by CORS, using fallback data");
  ...
  displayList = [...displayList].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));

  const topPriorityList = useMemo(() => {
    const pMap = { job: 3, academic: 2, activity: 1 };
    return [...notifications].sort((a, b) => {
      const diff = (pMap[b.category] || 0) - (pMap[a.category] || 0);
    ...
    <div style={{
      ...
      <header style={{
        ...
        <h2 style={{ margin: 0, color: '#111', fontSize: '24px' }}>
          Alerts Center{' '}
          <span style={{
            fontSize: '14px', background: '#ddd', padding: '2px 8px',
            borderRadius: '12px', verticalAlign: 'middle', marginLeft: '8px'
          }}>
            {stats.total}
          </span>
        </h2>
        <div style={{ display: 'flex', gap: '10px' }}>
          <button
            ...
            style={{
              padding: '8px 16px',
              background: viewMode === 'all' ? '#111' : '#fff',
              color: viewMode === 'all' ? '#fff' : '#111',
          ...
          <button
            ...
            style={{
              padding: '8px 16px',
              background: viewMode === 'top' ? '#111' : '#fff',
              color: viewMode === 'top' ? '#fff' : '#111',
              border: '1px solid #111',
              borderRadius: '6px',
              cursor: 'pointer',
              fontWeight: '500'
